...
import { guideCreateSchema, guideUpdateSchema } from "../schemas/guides.schema.js";
...
router.post("/guides", requireAuth, validate(guideCreateSchema), async (req, res) => {
    try {
...
router.put("/guides/:id", requireAuth, validate(guideUpdateSchema), async (req, res) => {
    try {
...
router.delete("/guides/:id", requireAuth, async (req, res) => {
    try {
        await prisma.guide.delete({ where: { id: Number(req.params.id) } });
